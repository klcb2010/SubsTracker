...
import { normalizeNotifyChannels } from '../services/notify/dispatch.js';
...
async function createSubscription(subscription, env) {
  try {
    ...
    const newSubscription = {
      ...
      useLunar: useLunar,
      notifyChannels: normalizeNotifyChannels(subscription.notifyChannels),
...
async function updateSubscription(id, subscription, env) {
  try {
    ...
    const merged = {
      ...
      useLunar: useLunar,
      notifyChannels:
        subscription.notifyChannels !== undefined
          ? normalizeNotifyChannels(subscription.notifyChannels)
          : existing.notifyChannels != null
            ? normalizeNotifyChannels(existing.notifyChannels)
            : null,
      updatedAt: new Date().toISOString()
    };
