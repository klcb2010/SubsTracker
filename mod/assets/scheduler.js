...
import { processOneTimeReminders } from './process-one-time.js';
...
export async function checkExpiringSubscriptions(env) {
  ...
  try {
    ...
    // 一次性定时提醒：不受「允许发送的小时」限制，到点（小时）即发
    try {
    ...
    // 按「实际渠道集合」分组后分别发送：
    // - 未设置 notifyChannels 的订阅 → 走全局渠道（key 用空串）
    // - 设置了的 → 只发指定渠道
    // 同组仍聚合为一条通知，保持原有体验
    /** @type {Map<string, typeof ready>} */
    const groups = new Map();
    for (const c of ready) {
      const ch = Array.isArray(c.sub.notifyChannels) && c.sub.notifyChannels.length > 0
    ...
    const entry = await schedulerLogsRepo.writeLog(env, {
      ...
      sentCount,
      autoRenewedCount,
      status: totalFailed > 0 && sentCount === 0 ? 'error' : 'ok',
      reason:
        totalAttempted > 0
          ? `发送到 ${totalAttempted} 个渠道调用，成功 ${totalSuccess} / 失败 ${totalFailed}（按渠道组分 ${groups.size} 组）`
          : '未启用任何通知渠道',
      extra: {
        candidates: ready.map((c) => ({
          ...
          daysDiff: c.daysDiff,
          notifyChannels: c.sub.notifyChannels || null
        })),
        channelResults: mergedChannelResults
      }
    });
    return entry;
  } catch (error) {
