// @ts-check
import * as otrRepo from './one-time-reminders.repo.js';
...
export async function handleOneTimeReminderRoutes(request, env, path) {
  ...
  if (!id && method === 'POST') {
    ...
    if (!content) return json({ success: false, message: '内容不能为空' }, 400);

    /** @type {import('./one-time-reminders.repo.js').OneTimeReminder} */
    const item = {
      id: otrRepo.makeId(),
      title,
      content,
  ...
  if (id && isTest && method === 'POST') {
    ...
    const result = await dispatch(
      ...
      config,
      { env, subId: `otr:${item.id}`, ruleId: 'one-time-test', logPrefix: '[定时提醒测试]', channels: item.channels }
    );
    return json({
